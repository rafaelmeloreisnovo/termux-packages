#include "real_sha256.h"
#include <stdio.h>
#include <string.h>

/* ============================================================================
 * REAL: SHA-256 per FIPS 180-4.
 * Verified against NIST test vectors:
 *   SHA256("") = e3b0c442 98fc1c14 9afbf4c8 996fb924 27ae41e4 649b934c
 *                a495991b 7852b855
 *   SHA256("abc") = ba7816bf 8f01cfea 414140de 5dae2223 b00361a3 96177a9c
 *                   b410ff61 f20015ad
 * ============================================================================ */

static const uint32_t k_K[64] = {
    0x428a2f98,0x71374491,0xb5c0fbcf,0xe9b5dba5,0x3956c25b,0x59f111f1,
    0x923f82a4,0xab1c5ed5,0xd807aa98,0x12835b01,0x243185be,0x550c7dc3,
    0x72be5d74,0x80deb1fe,0x9bdc06a7,0xc19bf174,0xe49b69c1,0xefbe4786,
    0x0fc19dc6,0x240ca1cc,0x2de92c6f,0x4a7484aa,0x5cb0a9dc,0x76f988da,
    0x983e5152,0xa831c66d,0xb00327c8,0xbf597fc7,0xc6e00bf3,0xd5a79147,
    0x06ca6351,0x14292967,0x27b70a85,0x2e1b2138,0x4d2c6dfc,0x53380d13,
    0x650a7354,0x766a0abb,0x81c2c92e,0x92722c85,0xa2bfe8a1,0xa81a664b,
    0xc24b8b70,0xc76c51a3,0xd192e819,0xd6990624,0xf40e3585,0x106aa070,
    0x19a4c116,0x1e376c08,0x2748774c,0x34b0bcb5,0x391c0cb3,0x4ed8aa4a,
    0x5b9cca4f,0x682e6ff3,0x748f82ee,0x78a5636f,0x84c87814,0x8cc70208,
    0x90befffa,0xa4506ceb,0xbef9a3f7,0xc67178f2
};

static const uint32_t k_H0[8] = {
    0x6a09e667,0xbb67ae85,0x3c6ef372,0xa54ff53a,
    0x510e527f,0x9b05688c,0x1f83d9ab,0x5be0cd19
};

#define ROR(x,n) (((x) >> (n)) | ((x) << (32 - (n))))
#define S0(x)    (ROR(x,2) ^ ROR(x,13) ^ ROR(x,22))
#define S1(x)    (ROR(x,6) ^ ROR(x,11) ^ ROR(x,25))
#define s0(x)    (ROR(x,7) ^ ROR(x,18) ^ ((x) >> 3))
#define s1(x)    (ROR(x,17) ^ ROR(x,19) ^ ((x) >> 10))
#define Ch(x,y,z) (((x) & (y)) ^ (~(x) & (z)))
#define Maj(x,y,z) (((x) & (y)) ^ ((x) & (z)) ^ ((y) & (z)))

static void sha256_transform(uint32_t state[8], const uint8_t block[64]) {
  uint32_t w[64];
  for (int i = 0; i < 16; i++) {
    w[i] = ((uint32_t)block[i*4] << 24) |
           ((uint32_t)block[i*4 + 1] << 16) |
           ((uint32_t)block[i*4 + 2] << 8) |
           ((uint32_t)block[i*4 + 3]);
  }
  for (int i = 16; i < 64; i++) {
    w[i] = s1(w[i-2]) + w[i-7] + s0(w[i-15]) + w[i-16];
  }

  uint32_t a = state[0], b = state[1], c = state[2], d = state[3];
  uint32_t e = state[4], f = state[5], g = state[6], h = state[7];

  for (int i = 0; i < 64; i++) {
    uint32_t t1 = h + S1(e) + Ch(e, f, g) + k_K[i] + w[i];
    uint32_t t2 = S0(a) + Maj(a, b, c);
    h = g; g = f; f = e; e = d + t1;
    d = c; c = b; b = a; a = t1 + t2;
  }

  state[0] += a; state[1] += b; state[2] += c; state[3] += d;
  state[4] += e; state[5] += f; state[6] += g; state[7] += h;
}

void real_sha256_init(real_sha256_ctx_t *ctx) {
  /* NONNULL contract enforced by compiler */
  memcpy(ctx->state, k_H0, sizeof(k_H0));
  ctx->bitcount = 0;
  ctx->buflen = 0;
}

void real_sha256_update(real_sha256_ctx_t *ctx, const uint8_t *data,
                        size_t len) {
  /* NONNULL_ALL contract enforced by compiler */
  ctx->bitcount += (uint64_t)len * 8;
  size_t take = REAL_SHA256_BLOCK - ctx->buflen;
  if (len >= take) {
    memcpy(ctx->buffer + ctx->buflen, data, take);
    sha256_transform(ctx->state, ctx->buffer);
    ctx->buflen = 0;
    data += take;
    len  -= take;
    while (len >= REAL_SHA256_BLOCK) {
      sha256_transform(ctx->state, data);
      data += REAL_SHA256_BLOCK;
      len  -= REAL_SHA256_BLOCK;
    }
  }
  memcpy(ctx->buffer + ctx->buflen, data, len);
  ctx->buflen += (uint32_t)len;
}

void real_sha256_final(real_sha256_ctx_t *ctx,
                       uint8_t out[REAL_SHA256_DIGEST]) {
  /* NONNULL_ALL contract enforced by compiler */
  uint64_t bits = ctx->bitcount;
  uint8_t pad = 0x80;
  real_sha256_update(ctx, &pad, 1);
  pad = 0x00;
  while (ctx->buflen != 56) real_sha256_update(ctx, &pad, 1);

  uint8_t lenbuf[8];
  for (int i = 0; i < 8; i++) lenbuf[i] = (uint8_t)(bits >> (56 - i * 8));
  real_sha256_update(ctx, lenbuf, 8);

  for (int i = 0; i < 8; i++) {
    out[i*4]     = (uint8_t)(ctx->state[i] >> 24);
    out[i*4 + 1] = (uint8_t)(ctx->state[i] >> 16);
    out[i*4 + 2] = (uint8_t)(ctx->state[i] >> 8);
    out[i*4 + 3] = (uint8_t)(ctx->state[i]);
  }
}

void real_sha256_buf(const uint8_t *data, size_t len,
                     uint8_t out[REAL_SHA256_DIGEST]) {
  /* NONNULL(1,3) contract; data may be NULL only if len==0 */
  real_sha256_ctx_t ctx;
  real_sha256_init(&ctx);
  if (len > 0) real_sha256_update(&ctx, data, len);
  real_sha256_final(&ctx, out);
}

int real_sha256_file(const char *path,
                     uint8_t out[REAL_SHA256_DIGEST],
                     uint64_t *out_size) {
  /* NONNULL_ALL contract */
  FILE *f = fopen(path, "rb");
  if (!f) return -1;
  real_sha256_ctx_t ctx;
  real_sha256_init(&ctx);
  uint8_t buf[8192];
  uint64_t total = 0;
  for (;;) {
    size_t n = fread(buf, 1, sizeof(buf), f);
    if (n == 0) break;
    real_sha256_update(&ctx, buf, n);
    total += n;
  }
  int err = ferror(f);
  fclose(f);
  if (err) return -1;
  real_sha256_final(&ctx, out);
  *out_size = total;
  return 0;
}

void real_sha256_hex(const uint8_t digest[REAL_SHA256_DIGEST],
                     char out[REAL_SHA256_HEXLEN]) {
  /* NONNULL_ALL contract */
  static const char h[] = "0123456789abcdef";
  for (int i = 0; i < REAL_SHA256_DIGEST; i++) {
    out[i*2]     = h[digest[i] >> 4];
    out[i*2 + 1] = h[digest[i] & 0x0f];
  }
  out[REAL_SHA256_DIGEST * 2] = '\0';
}
