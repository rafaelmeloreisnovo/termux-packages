...
class AptRepoGateTests(unittest.TestCase):
    def make_deb(self, base: Path, name: str = "bash", version: str = "1.0", arch: str = "aarch64") -> Path:
        ...
        (debian / "control").write_text(
            "\n".join([
                f"Package: {name}", f"Version: {version}", f"Architecture: {arch}",
        ...
        proc = subprocess.run(["dpkg-deb", "--build", "--root-owner-group", str(tree), str(out)], capture_output=True, text=True, check=False)
    ...
    def make_d3_receipt(self, receipts: Path, deb: Path) -> Path:
        ...
        doc = {
            "schema": "rafcodephi_delivery_gate/1.1.0", "gate": "artifact",
            "state": "PASS", "claim_allowed": False,
            "artifact_sha256": mod.sha256_file(deb),
            "package": mod.dpkg_field(deb, "Package"),
            "version": mod.dpkg_field(deb, "Version"),
            "arch": "aarch64", "debian_architecture": mod.dpkg_field(deb, "Architecture")
        }
    ...
    def test_d4_requires_matching_d3_receipt(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            base = Path(td); artifacts = base / "artifacts"; receipts = base / "receipts"
            artifacts.mkdir(); self.make_deb(artifacts)
            with self.assertRaises(mod.GateError): mod.collect_artifacts(artifacts, receipts, "aarch64")
            receipts.mkdir()
            with self.assertRaises(mod.GateError) as ctx: mod.collect_artifacts(artifacts, receipts, "aarch64")
    ...
    def test_valid_d3_artifact_generates_deterministic_metadata(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            base = Path(td); artifacts = base / "artifacts"; receipts = base / "receipts"
            artifacts.mkdir(); deb = self.make_deb(artifacts); self.make_d3_receipt(receipts, deb)
            records = mod.collect_artifacts(artifacts, receipts, "aarch64")
            out1 = base / "repo1"; out2 = base / "repo2"
            r1 = mod.generate_repo(records, out1, "aarch64", "stable", "main", 0)
            r2 = mod.generate_repo(records, out2, "aarch64", "stable", "main", 0)
            mod.validate_generated_repo(out1, r1); mod.validate_generated_repo(out2, r2)
    ...
    def test_tampered_artifact_breaks_receipt_binding(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            base = Path(td); artifacts = base / "artifacts"; receipts = base / "receipts"
            artifacts.mkdir(); deb = self.make_deb(artifacts); self.make_d3_receipt(receipts, deb)
            with deb.open("ab") as fh: fh.write(b"tamper")
            with self.assertRaises(mod.GateError) as ctx: mod.collect_artifacts(artifacts, receipts, "aarch64")
            self.assertIn("lacks matching PASS D3 receipt", str(ctx.exception))
    ...
    def test_receipt_metadata_mismatch_is_blocked(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            base = Path(td); artifacts = base / "artifacts"; receipts = base / "receipts"
            artifacts.mkdir(); deb = self.make_deb(artifacts); receipt = self.make_d3_receipt(receipts, deb)
            doc = json.loads(receipt.read_text(encoding="utf-8")); doc["package"] = "not-bash"
    ...
    def test_missing_artifact_set_blocks_d4(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            base = Path(td); artifacts = base / "artifacts"; receipts = base / "receipts"
            artifacts.mkdir(); receipts.mkdir()
            with self.assertRaises(mod.GateError) as ctx: mod.collect_artifacts(artifacts, receipts, "aarch64")
