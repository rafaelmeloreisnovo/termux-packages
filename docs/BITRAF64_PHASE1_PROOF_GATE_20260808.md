...
This hotfix replaces the lossy Phase-1 integration already present on `main` while refusing claims not supported by the implementation.

## Why this branch exists

The existing Phase-1 code describes a 33-bit `raf_sig` and 48-bit packed metadata value, but Manifest V2 exposes only a 32-bit `_reserved` field while preserving a fixed 200-byte ABI. The old write therefore truncated information and described overflow that was not materialized.

The corrected implementation keeps Manifest V2 unchanged and adds a sidecar:
...
Executable regressions enforce:
...
3. Manifest V2 remains exactly 200 bytes.
4. A package payload mutation invalidates the recomputed Phase-1 tag/sidecar pair.
5. The gate fails closed while mathematical and empirical proof obligations remain absent.
...
`structural integrity -> GF(2) rank fixtures -> ECC error-injection corpus -> real package DAG mapping -> entropy/autocorrelation measurement -> homogeneous SIMD benchmark -> independent replication -> selective claim review`
