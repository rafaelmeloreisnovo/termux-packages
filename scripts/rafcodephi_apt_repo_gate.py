...
def receipt_index(receipts_dir: Path) -> dict[str, tuple[Path, dict[str, Any]]]:
    ...
    for path in sorted(receipts_dir.rglob("*.json")):
        binding = extract_artifact_binding(load_json(path))
        if binding is None:
            continue
...
def collect_artifacts(artifacts_dir: Path, receipts_dir: Path, arch: str) -> list[DebRecord]:
    ...
    for deb in debs:
        ...
        if architecture not in {arch, "all"}:
            raise GateError(f"artifact architecture mismatch: {deb.name}: {architecture!r} not {arch!r}/'all'")
        for key, actual in (("package", package), ("version", version)):
            claimed = binding.get(key)
            if claimed is not None and claimed != actual:
                raise GateError(f"D3 receipt metadata mismatch for {deb.name}: {key}={claimed!r} != {actual!r}")
        ...
        if claimed_arch is not None and claimed_arch not in {architecture, arch}:
            raise GateError(f"D3 receipt architecture mismatch for {deb.name}: {claimed_arch!r}")
        identity = (package, version, architecture)
        if identity in identities:
            raise GateError(f"duplicate package/version/architecture identity: {identity}")
        identities.add(identity)
        records.append(DebRecord(deb, package, version, architecture, digest, deb.stat().st_size, dpkg_control(deb), receipt_path))
...
def package_paragraph(record: DebRecord, filename: str) -> str:
    return strip_generated_fields(record.control) + f"Filename: {filename}\nSize: {record.size}\nSHA256: {record.sha256}\n"
...
def rel_hash_line(path: Path, root: Path) -> str:
    return f" {sha256_file(path)} {path.stat().st_size:16d} {path.relative_to(root).as_posix()}"


def generate_repo(records: Iterable[DebRecord], out_dir: Path, arch: str, suite: str, component: str, epoch: int) -> dict[str, Any]:
    ...
    for record in records:
        ...
        artifact_receipts.append({"package": record.package, "version": record.version, "architecture": record.architecture, "filename": rel, "size": record.size, "sha256": record.sha256, "d3_receipt": str(record.receipt)})
    ...
    release.write_text("\n".join(["Origin: RAFCODEPHI", "Label: RAFCODEPHI", f"Suite: {suite}", f"Codename: {suite}", f"Date: {release_date(epoch)}", f"Architectures: {arch}", f"Components: {component}", "Description: RAFCODEPHI validated package repository metadata", "SHA256:", *[rel_hash_line(p, release.parent) for p in indexed], ""]), encoding="utf-8")
    ...
    return {
        ...
        "indices": {p.relative_to(out_dir).as_posix(): {"size": p.stat().st_size, "sha256": sha256_file(p)} for p in [packages, packages_gz, packages_xz, release]},
        ...
        "next_gate": "validate repository trust/signature policy and run apt metadata consumption in an isolated client before Android promotion",
    }
...
def validate_generated_repo(out_dir: Path, receipt: dict[str, Any]) -> None:
    ...
    for rel, meta in indices.items():
        ...
        if meta.get("size") != path.stat().st_size or meta.get("sha256") != sha256_file(path):
            raise GateError(f"generated metadata drift: {rel}")
    ...
    paragraphs = [p for p in (out_dir / packages_rel).read_text(encoding="utf-8").strip().split("\n\n") if p.strip()]
    ...
    for paragraph in paragraphs:
        for field in ("Package:", "Version:", "Architecture:", "Filename:", "Size:", "SHA256:"):
            if not any(line.startswith(field) for line in paragraph.splitlines()):
